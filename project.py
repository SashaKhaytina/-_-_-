...
class Menu:
    ...
    def Redact_def(self, update, context):
        update.message.reply_text(
            "Всем привет! Я бот, который поможет вам хорошо учиться! "
            "1.Чтобы не носить много учебников или если вдруг он поттерялся, не беда!"
            "Нажмите на кнопку Subject и откройте ваш учебник!"
            "2. Нужно что-то выучить, но спросить вас никто не может спросить?"
            "Я спрошу тебя! Только расскажии мне какие вопросы тебе задавать."
            "Это можно сделать нажав на кнопку Repetition. "
            "Пройдите тест нажав на кнопку Repetition_start и посмотрите результаты прошлого теста в кнопке Delete_test",
            reply_markup=self.markup_reda
        )
